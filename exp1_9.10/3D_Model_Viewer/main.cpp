		...
		while (! fileObject.eof()){
			...
			if (fileObjectLine.c_str()[0] == 'f'){				// if the line starts with 'f', it's declaring a face/polygon
				...
				while(iss){
					...
					// Skip texture and normal indices (format: vertex/texture/normal or vertex//normal)
					if (iss.peek() == '/') {
						iss.ignore(); // skip first '/'
						if (iss.peek() == '/') {
							// Format: vertex//normal
							iss.ignore(); // skip second '/'
							int normalIndex;
							iss >> normalIndex; // read normal index but don't use it
						} else {
							// Format: vertex/texture/normal
							int textureIndex, normalIndex;
							iss >> textureIndex; // read texture index but don't use it
							iss.ignore(); // skip '/'
							iss >> normalIndex; // read normal index but don't use it
						}
					}
